...
app.get('/:namet',function(req,resp){
 ...
 if(err) throw err;
    ...
    console.log(uri);
    var cursor = db.collection('link').find({"_id":uri}).limit(1);
...
app.get('/delete/:namet',function(req,resp){
    mongo.connect(url,function(err,db){
        if(err) throw err;
            var uri = req.params.namet;
        var find = db.collection('link').findOne({_id:uri},
        function(err,data){
            if(err) throw data;
            if(!data){
                resp.end("Link doesn't exists");
                 ...
                 var cursor = db.collection('link').deleteOne({_id:uri},
...
mongo.connect(url, function(err, db) {
 if(err) throw err;
 var uri = req.params.namet;
  var cursor =db.collection('link').find({"old":uri}).limit(1);
  var count = 0;
   cursor.each(function(err, doc) {
       if(count ==0){
           count++;
